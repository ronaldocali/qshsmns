...
@section('content')
    <div class="roles-permissions">
        ...
        <div class="mt-8">
            <div class="flex flex-wrap items-center uppercase text-sm font-semibold theHeaderClasses">
            ...
            @foreach ($classes as $class)
                <div class="flex flex-wrap items-center theClassesRecord">
                    <div class="w-1/12 px-4 py-3 text-sm font-semibold tracking-tight">{{ $class->class_numeric }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold tracking-tight">{{ $class->class_name }}</div>
                    <div class="w-1/12 px-4 py-3 text-sm font-semibold tracking-tight">
                        <span class="bg-gray-200 text-sm mr-1 mb-1 px-2 font-semibold border rounded-full">
                            {{ $class->students_count }}
                        </span>
                    </div>
                    <div class="w-4/12 px-4 py-3 text-sm text-center tracking-tight">
                        @foreach ($class->subjects as $subject)
                            <span class="bg-gray-200 text-sm mr-1 mb-1 px-2 font-semibold border rounded-full">{{ $subject->subject_code }}</span>
                        @endforeach
                    </div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold tracking-tight">{{ $class->teacher->user->name ?? '' }}</div>
