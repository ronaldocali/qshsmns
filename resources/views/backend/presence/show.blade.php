...
@section('content')
    <div class="create">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">Presence</h2>
            </div>
        ...
        <div class="w-full mt-8 bg-white rounded">
            <div class="flex items-center justify-between px-6 py-6 pb-0">
                <div class="border-l-4 border-gray-600 pl-2">
                    <h2 class="text-gray-700 uppercase font-bold">{{ $presences[0]->student->user->name ?? '' }}</h2>
                </div>
            </div>
            ...
            <div class="w-full px-6 py-6">
                <div class="flex items-center justify-between bg-gray-200 rounded-tl rounded-tr">
                    ...
                    <div class="w-3/12 text-right text-gray-600 py-2 px-4 font-semibold">Presence</div>
                </div>
                @foreach ($presences as $presence)
                    <div class="flex items-center justify-between border border-gray-200">
                        <div class="w-3/12 text-sm text-left text-gray-600 py-2 px-4 font-semibold">{{ $presence->presence_date }}</div>
                        <div class="w-3/12 text-sm text-left text-gray-600 py-2 px-4 font-semibold">{{ $presence->teacher->user->name }}</div>
                        <div class="w-3/12 text-sm text-left text-gray-600 py-2 px-4 font-semibold">{{ $presence->class->class_name }}</div>
                        <div class="w-3/12 text-xs text-right text-gray-600 py-2 px-4 font-semibold">
                            @if ($presence->presence_status)
